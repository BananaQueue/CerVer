...
**Photo of a page (on by default since 2026-08-25).** After a seal verifies,
"Attach a photo of this page" checks every amount, date, duration, reference,
and citation on the record against what the camera read. This is **evidence,
not proof**: OCR is lossy, so a clean result reads "nothing found", never
"verified" — it narrows the honest limit below without removing it. Spec
§9.2's shipping criteria (zero material false positives across real
photographs, not renders) are measured and hold — see the spec's final
calibration record. `CERVER_PAGE_IMAGE_OCR=0` is a kill switch if the feature
ever needs to be pulled without a code change:

```bash
CERVER_PAGE_IMAGE_OCR=0 npm start
```
